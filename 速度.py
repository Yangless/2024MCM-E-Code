...
# 加载YOLOv5模型
model = torch.hub.load('ultralytics/yolov5:v6.0', 'yolov5m')

# 导入视频
txt_name='3.2.speed.txt'
video_path = '3.2.mp4'  # 视频文件路径
...
while cap.isOpened():
    ...
    # 过滤出车辆检测结果
    cars = detections[detections['name'].isin(['car', 'truck'])]  # 筛选出车辆
    now_mess = []
    now_cross = 0
    fz=0
    fm=0
    ...
    for index, row in cars.iterrows():
        ...
        # 检查车辆是否已通过并未被统计
        # if center_y > line_position and car_box not in passed_cars:
        #     total_car_count += 1  # 累加到总车辆计数
        #     passed_cars.add(car_box)  # 将车辆框添加到已通过集合中
        ...
        for t in range(max(0, num - 1 - T), num - 1):
            # print(range(max(0, num - 1 - T), num - 1))
            # print("t=",t)
            for i in range(len(mess[t])):
                ...
                if superposition(car_x, car_y, car_w, car_h, old_car[0], old_car[1], old_car[2], old_car[3]) >= max_area:
                    min_t, min_i, max_area = t, i, superposition(car_x, car_y, car_w, car_h, old_car[0], old_car[1], old_car[2],
                                                                 old_car[3])
        if max_area <= Area:
            for t in range(max(0, num - 1 - T), num - 1):
                for i in range(len(mess[t])):
        ...
        if max_area >= Area:
            ...
            if car_size>3000:
                speed=(((car_x-old_car[0])**2+(car_y-old_car[1])**2)**0.5)*calc_k(car_x,car_y)/(1/25*(num-min_t))/10
                speed=abs(speed)
                if speed>0.01:
                    fz+=speed
                    fm+=1
        ...
        frame = cv2.rectangle(frame, (int(row.xmin), int(row.ymin)),
                               ...
                               car_color, 2)
        # 在矩形框的左上角显示车辆ID
        cv2.putText(frame, f'ID: {now_car[4]} V={speed:.2f}/{speed*3.6:.2f}', (int(row.xmin), int(row.ymin) - 10),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)


    txt.append([fz/(fm+0.0001),fm])
    mess.append(now_mess)
    print("frame = ", num, "ID =", ID, "Cross = ", len(cross))
    # 在视频帧上显示当前车辆计数
    cv2.putText(frame, f'Total Car Count: {len(cross)} , V_avg={fz/(fm+0.0001):.2f}/{fz/(fm+0.0001)*3.6:.2f}',
